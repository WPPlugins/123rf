...
jQuery(document).ready(function($) {
  var $back_button, $button_license_accept, $button_license_reject, $detail, $images, $nb_images, $paging, $search_button, $search_category, $search_filter, $search_input, $search_input_copy, $search_sort, $search_type, $search_orientation, $search_sync, $ui_detail, $ui_search, bindImages, bindLicenseTable, bindPaging, buyImage, currentStatus, getPopularImages, isUserLogged, mpp_checkStatus, mpp_syncSearch, mpp_syncSearch1, pd, scroll_position, search;
  pd = parent.document;
  scroll_position = 0;
  ...
  $filter_button = $('input[name=mpp_filter_button]');
  $search_sync = $('input[name=mpp_search_sync]');
  $search_filter = $('.mpp_search_filter_checkbox');
  $search_sort = $('select[name=mpp_sort]');
  $search_type = $('select[name=mpp_type]');
  $search_orientation = $('select[name=mpp_orientation]');
  $search_category = $('select[name=mpp_category]');
  ...
  mpp_checkStatus = function(refresh) {
    ...
    if (refresh || !currentStatus) {
      return $.post(Plugin123RF.ajax_url, {
        a: 'getUserData',
        module: Plugin123RF.module
      }, function(r) {
        var _ref;
        ...
        if (parent.mpp_currentState === 'mpp-' + Plugin123RF.module) {
  ...
  buyImage = function(license_agreed) {
    ...
    return $.post(Plugin123RF.ajax_url, {
      ...
      if (r.status === 1 && (r.id != null)) {
        ...
        return tb_show(Plugin123RF.text.license_agreement, '#TB_inline?width=500&modal=true&height=400&inlineId=mpp_license_dialog', false);
      } else if (r.status === 2) {
        $('input[name=mpp_license]').attr('disabled', false);
        $('input[name=mpp_buy]').attr('disabled', false).removeClass('button-primary').addClass('button-secondary').val(Plugin123RF.text.buy);
        if (r.message) {
          return $('.mpp_detail_error_message').html(r.message);
        } else {
          return $('.mpp_detail_error_message').text(Plugin123RF.text.error_purchase);
        }
      }
    });
  };
  bindLicenseTable = function() {
    if (!isUserLogged) {
      ...
      $('input[name=mpp_buy]').attr('disabled', false).attr('data-link', Plugin123RF.options_url).val(Plugin123RF.text.please_login);
      if (Plugin123RF.module_register_link) {
        $('a.mpp_register').show().attr('href', Plugin123RF.module_register_link);
      }
      if (Plugin123RF.module_detail_offer) {
        $('.mpp_offer_detail').html(Plugin123RF.module_detail_offer).show();
      }
    }
    ...
    $('.mpp_license_row').bind('click', function() {
      var $t;
      $(".mpp_license_row").css("background", "");
      $t = $(this).find('input[name=mpp_license]');
      if (!$t.attr('disabled')) {
        $('input[name=mpp_buy]').attr('disabled', false).removeClass('button-primary').addClass('button-secondary').val(Plugin123RF.text.buy);
        document.getElementById("mpp_row_" + $t.val()).style.background = "#F5F6CE";
        return $t.attr('checked', true);
      }
    });
    $('input[name=mpp_license]').bind('click', function() {
      if (!$(this).attr('disabled')) {
        return $('input[name=mpp_buy]').attr('disabled', false).removeClass('button-primary').addClass('button-secondary').val(Plugin123RF.text.buy);
      }
    });
    return $('input[name=mpp_buy]').bind('click', function() {
      ...
      if (!$t.attr('disabled') && !$t.attr('data-link')) {
        if ($t.hasClass('button-secondary')) {
          $(this).removeClass('button-secondary').addClass('button-primary').val(Plugin123RF.text.confirm);
        } else {
          $('input[name=mpp_license]').attr('disabled', true);
  ...
  search = function(text, page) {
    ...
    return $.post(Plugin123RF.ajax_url, {
      ...
      orientation: $search_orientation.val(),
      category: $search_category.val(),
      models: $('select[name=mpp_models]').val(),
      language: $('select[name=mpp_language]').val(),
      people: $('select[name=mpp_people]').val(),
      age: $('select[name=mpp_age]').val(),
      gender: $('select[name=mpp_gender]').val(),
  ...
  $filter_button.bind('click', function() {
    if($(this).val() == "Show filters"){
      document.getElementById("mpp_filters").style.display = "block";
      $(this).val("Hide filters");
    } else {
      document.getElementById("mpp_filters").style.display = "none";
